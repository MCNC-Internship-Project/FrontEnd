...
<script setup>
/**
 * @fileoverview  설문 항목의 타입별(객관식) 질문 항목 컴포넌트
 * @author 김원재 (kimwonjae@mcnc.co.kr)
 * @date 2024-11-08
 * @lastModified 2024-12-19
 * @description 질문 항목에 대한 화면 구현.
 */
...
// 설문 항목 당 질문 항목들을 담는 배열
const totalItem = ref([
    { id: 0, value: "", isDuplicated: false, hasError: false },
]);
...
const addItem = () => {
    ...
    if (isExistEtc.value) { // 기타항목이 있으면 기타항목을 최하위로 정렬
        const etcItem = totalItem.value.find(item => item.id === 'etcId');
...
const getValue = () => {
    ...
    checkEmptyValueArray.forEach((value, index) => {
        if (value === "") { // 질문 항목의 내용이 비었을 경우
            totalItem.value[index].hasError = true;
            totalItem.value[index].value = "";

            if (initIndex === -1) {
                initIndex = index; // 초기 index 값을 해당 index 값으로 변경
            }
        }
    });
...
/**
 * SurveyItem의 objComponentRef에서 호출하는 자식 컴포넌트의 getValue 함수를 접근 가능하게 함
 * @author 김원재
 */
defineExpose({
    getValue,
